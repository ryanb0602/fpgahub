...
GIT_PORT = 9418
REPO_NAME = "eth_mac_phy_10g_fifo.git"
...
def run_once(run_dir):
    ...
    # Set the server's symbolic HEAD to 'main' so push/clone don't complain about branch mismatches
    subprocess.run(
        [
            "docker",
            "exec",
    ...
    # 3. Clean slate for ./veth_v1/ and ./veth_v2/ to prevent cache collisions
    shutil.rmtree("./veth_v1/.git", ignore_errors=True)
    shutil.rmtree("./veth_v1/.fpgahub", ignore_errors=True)
    shutil.rmtree("./veth_v2/.git", ignore_errors=True)
    shutil.rmtree("./veth_v2/.fpgahub", ignore_errors=True)

    # 4. Seed initial baseline from ./veth_v1/
    print("[init] Seeding initial HDL baseline from ./veth_v1/...")
    subprocess.run(
        ["git", "init"], cwd="./veth_v1/", check=True, stdout=subprocess.DEVNULL
    )
    subprocess.run(
        ["git", "config", "user.name", "Benchmark User"], cwd="./veth_v1/", check=True
    )
    subprocess.run(
        ["git", "config", "user.email", "benchmark@rpi.edu"],
        cwd="./veth_v1/",
        check=True,
    )
    subprocess.run(["git", "add", "."], cwd="./veth_v1/", check=True)
    subprocess.run(
    ...
    subprocess.run(["git", "branch", "-M", "main"], cwd="./veth_v1/", check=True)
    subprocess.run(
        ["git", "remote", "add", "origin", REMOTE_URL], cwd="./veth_v1/", check=True
    )
    subprocess.run(
    ...
    # --- PHASE 1: CLONE (Pulling initial state over native git:// protocol) ---
    results.append(
        run_and_monitor(
            ["git", "clone", "-b", "main", REMOTE_URL, "./temp_dir/"],
            "clone",
            "./",
            proc_log,
        )
    ...
    # --- PHASE 2: TRANSFER METADATA to ./veth_v2/ ---
    # Replaces copying .fpgahub so ./veth_v2/ becomes an active Git working tree
    shutil.copytree("./temp_dir/.git", "./veth_v2/.git")
    subprocess.run(
        ["git", "config", "user.name", "Benchmark User"], cwd="./veth_v2/", check=True
    )
    subprocess.run(
        ["git", "config", "user.email", "benchmark@rpi.edu"],
        cwd="./veth_v2/",
    ...
    # --- PHASE 3: STAGE & COMMIT inside ./veth_v2/ ---
    results.append(
        run_and_monitor(
            ["git", "add", "."],
            "add",
    ...
    results.append(
        run_and_monitor(
            ["git", "commit", "-m", "Benchmark eth_mac_phy_10g_fifo change"],
            "commit",
            "./veth_v2/",
            proc_log,
        )
...
def main(num_runs=30):
    batch_root = f"runs_eth_daemon_{datetime.now().strftime('%Y%m%d_%H%M%S')}"
