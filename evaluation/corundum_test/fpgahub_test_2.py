...
def sample_tree(procs):
    ...
    NOTE: cpu_percent(None) is non-blocking but only meaningful after an
    earlier "priming" call was already made on that same Process object,
    which is why we prime every new process the moment we see it.
    """
    total_cpu = 0.0
    total_mem = 0.0
    alive = []
...
def get_container_cgroup_paths(container_name):
    """
    Resolve the cgroup v2 paths for a running container so we can poll
    cpu.stat / memory.current directly instead of going through the
    `docker stats` CLI (which is hard-capped at ~1Hz by the Docker daemon).
    """
    ...
    # Common cgroup v2 layout. Adjust the base path if your host uses a
    # different driver (e.g. cgroupfs vs systemd) or cgroup v1.
    candidates = [
...
def monitor_docker_stats(container_name, log_file, stop_event, interval=0.02):
    """
    Polls the container's cgroup accounting files directly at `interval`
    seconds. This gets you real sub-second resolution, unlike the
    `docker stats` CLI which only refreshes ~once/second internally.
    cpu_percent here is computed as (delta cpu time) / (delta wall time),
    normalized by host core count, same convention `docker stats` uses.
    """
    ...
    with open(log_file, "w", newline="") as f:
        ...
        while not stop_event.is_set():
            ...
            if cgroup_path:
                ...
                # Fallback: original docker stats CLI approach (coarse, ~1Hz max)
                result = subprocess.run(
                    [
                        "docker",
                        "stats",
...
def _exitcode_from_status(status):
    """os.waitstatus_to_exitcode exists on py3.9+; small fallback for older pythons."""
...
def run_and_monitor(cmd, phase_name, cwd, proc_log_file, poll_interval=0.02):
    ...
    def is_running(pid):
        """
        Checks liveness WITHOUT reaping. Deliberately avoids proc.poll(),
        because Popen.poll() calls waitpid(WNOHANG) under the hood and will
        reap the zombie itself the instant it detects exit -- which steals
        the process from our later os.wait4() call and makes it always fail
        with ChildProcessError.
        """
        try:
    ...
    with open(proc_log_file, "a", newline="") as f:
        ...
        while is_running(proc.pid):
            # pick up any children spawned since last check
    ...
    # Loop exited because the process became a zombie (or vanished). Nothing has
    # reaped it yet since we never called proc.poll(). Reap it ourselves now via
    # os.wait4 to get the KERNEL'S EXACT resource usage for this process --
    # authoritative accounting, immune to the sampling race, and not racing
    # with Popen's own internal reaping.
